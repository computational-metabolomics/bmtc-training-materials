...
- install R so that you can write code and run examples
- install RStudio which provides a friendlier interface in which to run and develop code
- install R packages required for future modules.
...
- Under Installers select RStudio x.yy.zzz - Windows Vista/7/8/10 (where x, y, and z represent version numbers) **The current release is R version 4.4.0**.
...
- Go to the RStudio download page
- Under Installers select RStudio x.yy.zzz - Mac OS X 10.6+ (64-bit) (where x, y, and z represent version numbers) **The current release is R version 4.4.0**.
...
Compare your R version to the version listed [here](https://www.bioconductor.org/install/) for the current release. **The current release of Bioconductor is version 3.20; it works with R version 4.4.0.**
...
After installing a new version of R, you will have to re-install **all** packages with the new version.
